...
const { SG, corpusGraphs, goldenGraphs, g3aGraphs, storedScores, scoreOf, withPositions } = require('./helpers.js');
...
test('A48: every corpus Score comes back from its rebuilt graph as the same music', async () => {
  ...
  assert.ok(graphs.length >= 360, graphs.length + ' corpus graphs');
  const failures = [];
