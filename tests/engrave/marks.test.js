...
test('versions (G4-D1a-1): every change to what the plan and the layout output moves their version - plan/3, engr/4 (G4d-1b), engr/5 (its fixer), engr/6 (G4e print), engr/7 (G4-L7) - and the layout says whose plan it drew', async () => {
  const x = await lay('E01');
  assert.equal(x.p.version, 'plan/3');
  assert.equal(E.PLAN_VERSION, 'plan/3');
  assert.equal(x.e.version, 'engr/7');
