...
| `lessons.test.js` | Piano Basics, the beginner course: every lesson well formed and finishable (in node), every word in Korean, Japanese and Chinese — then in the page the tab under Home, lesson 1 finished by clicking (a wrong key, the find step, a quiz, XP once), A S D F as Do Re Mi without the practice shortcuts firing, a MIDI note as an answer, Listen lighting the keys, the staff drill, a chord, a rhythm tapped on time and one that is not, progress kept across a reload, the page in Korean, and a phone with no sideways scroll. |
