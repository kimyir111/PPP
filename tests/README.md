...
| `transcription.test.js` | Recordings → scores. Performances built from known scores (a waltz, an Alberti bass, a rubato waltz) played with human untidiness, checking that the metre, tempo, key, downbeat, hands and bar count are recovered; spelling; note values; the parser, the engraving and the confidence report. With the helper running and transcription set up, a synthesised recording goes through the real UI to the review screen, plays back by measure, and joins My Songs. Skips that half cleanly otherwise. |
