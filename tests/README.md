...
| `share.test.js` | Shared Scores: the tab, a Share button on your songs but not the sample, a guest asked to sign in, a link made only when first sent (X and Facebook get it; `window.open` is recorded, never followed), sending a link not posting, only the notes going to the server, Post and Take down, the Open Graph tags on the link, someone with no account landing on the score and adding it to their songs, only the owner changing a share, and Stop sharing asking first and ending the link. Needs the Node server; skips if it has no `/api/shares`. |
...
| `lessons.test.js` | Piano Basics, the course in three levels: every lesson well formed and finishable (in node) — chords inside a line, ties, 3/4, flat spelling, per-level numbering — and every word in Korean, Japanese and Chinese; then in the page the level picker (three levels, Start or Continue, resume after a reload, ← All levels, the level tabs, the end of level 1 leading to level 2), chord progressions lit and waited for, both hands for bass and chord, key signatures, an ear question with its own Listen, a waltz with a three-click count-in and a tied rhythm tapped on time, the tab under Home, lesson 1 finished by clicking (a wrong key, the find step, a quiz, XP once), A S D F as Do Re Mi without the practice shortcuts firing, a MIDI note as an answer, Listen lighting the keys, the see-through hand marking the finger to play (moving on as you play, the thumb passing under in the scale, the left hand, a written fingering, a chord's three fingers, nothing marked when the step gives no hints, the Show hands toggle shared with the practice screen), the staff drill, a chord, a rhythm tapped on time and one that is not, progress kept across a reload, the page in Korean, and a phone with no sideways scroll. |
| `hymns-share.test.js` | The 100 찬송가 piano scores in `catalog/hymns`: distinct Korean titles, each MusicXML parsed by `PPP.parseMusicXML` as measures with sounding notes in both hands, then one posted listed through the real `/api/shares` still carrying its YouTube watch URL. |
| `course.test.js` | Method Books: in node, the catalog (every score in the repository, numbers inside each book, bars/key/time for each, every word in Korean, Japanese and Chinese) and the course rules (today's plan with warm-up, review and a book alongside, circles filled and taken back, passing and skipping, the streak across days and month ends, a finished book naming the next, saved state cleaned); in the page, the tab under Piano Basics, the path in academy order, choosing a book, circles tapped and counted, 3 circles a day, Pass moving the book on and marking the grid, all of it kept across a reload, Practice opening the piece and adding it to My Songs once, every score in the catalog read by `PPP.parseMusicXML` with notes for each hand, Start from here, Korean, and a phone with no sideways scroll. |
...
```sh
npm run bench:smoke     # 32 cases, a few seconds
...
npm run test:bench      # the benchmark's own unit tests and the golden snapshots
```
