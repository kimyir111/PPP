...
**상태: 구현 진행 중 (병렬로 §48·§49도 진행).**

## 48. G4 폴리싱 — 소나티네 빽빽한 구간 beam (조사, 2026-09-27)

M-H2 §42.2의 관찰(빽빽한 소나티네 층 4개 중 2개에서 legacy가 "전체" 점수로 이겼다)을 별도 워크트리(`D:/PPP-g4-sonatina`)에서 조사한다 — 이번엔 미리 결론을 정하지 않고, 실제 결함이 있으면 고치고 없으면 §46처럼 조사만으로 닫는다. 브리프 `brief_g4_sonatina.md`.

## 49. G4 폴리싱 — B5 첫 그리기 long task (2026-09-27)

G4d-2부터 있던 §16.3 미해결 항목(전체 악보 첫 그리기가 한 덩어리 long task). 별도 워크트리(`D:/PPP-g4-b5`)에서 진행. 브리프 `brief_g4_b5.md`.
