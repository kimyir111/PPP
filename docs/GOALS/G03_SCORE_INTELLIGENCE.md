...
## 30. A36 사람 평가 결과 (2026-09-24)

`D:/PPP-g3`, `f607728`. 평가자 1명(사용자, 역할 미기재), 2026-09-24. 결과 파일 `tests/bench/human/g3/review-2026-09-24.json` — 사용자가 내보낸 파일 **그대로**(sha256 `9af40041…478e`, `cmp` 동일). 판정·메모를 고치지 않았고 빈 `reviewer_role`·`renderer`도 그대로 두었다 (렌더러는 §30.2에 기록). 열쇠는 평가 제출 뒤에만 열었다.

### 30.1 판정: **A36 FAIL** → G3a flip 하지 않음

`python tests/bench/run.py human-set --score tests/bench/human/g3/review-2026-09-24.json` (exit 1):

| §21.5 기준 | 필요 | 결과 |
| --- | --- | --- |
| 모든 발췌 4축 순위, 60 파일 평가 | 빈칸 0 | PASS (0, 0) |
| 전체 축: G3a ≥ G3 off | 20개 중 18 이상 | **FAIL — 16** |
| 리듬 축: G3a가 G3 off보다 나쁨 | 0 | **FAIL — 4** (E07, E11, E15, E17) |
| 절대 평가: G3a의 yes+fix > G3 off의 yes+fix | B > A | **FAIL — 12 = 12** (참조 16) |
| `nq.ned` A → B (정보) | 감소 기대 | 1.0225 → 1.0383 (증가), Kendall τ −0.046 |
| 참조가 전체 축 1위(같은 순위 포함) (정보) | | 20 / 20 |

따라서 `PROFESSIONAL_DEFAULT = 'off'` 유지. golden bless, `nq.*` core baseline, mutation `G3_ON` 정리, PR — 전부 하지 않았다. G3b·자동 8va·`pedalJoin` OFF 그대로. 평가자 1명이라는 한계는 §21.4.5대로 결과에 적는다.

### 30.2 무엇이 평가됐나

- **평가한 파일 = 현재 G3a 출력.** `f607728`에서 `human-set --build`를 다시 돌려 60 MusicXML이 바이트 동일함을 확인했다. 달라진 것은 manifest·key·form의 commit 표기(`1c030f23b1` → `f607728bdf`)뿐이라 되돌렸다.
- **렌더러: D7(a) 이탈 (DECISIONS G3-U8).** 사용자 지시로 MuseScore 4 대신 앱 렌더러(ScoreView, VexFlow 4.2.3)를 쓰는 dev 페이지(`?devReview=g3`)에서 평가했다. 이 도구는 로컬 브랜치 `g3-dev-review-tool`(`9d02842`, push 안 함)에만 있고 이 브랜치·main에는 없다. 영향은 셋이다:
  1. 앱은 파일의 `<beam>`을 무시하고 스스로 묶는다 (D4). G3 off는 beam을 하나도 쓰지 않고(0) G3a는 3,305개를 쓰는데, 이 차이는 **평가에서 보이지 않았다**. tuplet 괄호(F1)도 앱이 스스로 정한다.
  2. 쉼표·음가·tie·셋잇단 값·staff·성부는 파일 그대로 그린다. 진 6 발췌에서 앱이 그린 쉼표 수(`data-rest`)는 판본마다 파일의 `<rest>` 수와 정확히 같다 — G3 off → G3a: E07 38 → 39, E11 26 → 33, E12 45 → 57, E13 71 → 77, E15 44 → 50, E17 67 → 63. **리듬 축 패배는 렌더러가 만든 것이 아니다.**
  3. 메모 둘은 참조(C)를 그린 앱 렌더러의 결함이다 (E05 "되돌이표 위치 어긋남", E19 "음표가 두 개씩 이어짐" = 앱의 beam). A 대 B 판정과는 무관하다.
- **세트의 한계 (평가 뒤 발견, 판정에 쓰지 않음).** 녹음 경로(A·B 공통)의 박자가 참조와 다른 발췌 7개 (E01, E02, E03, E07, E12, E13, E16 — 예: 6/8 대 2/4), 조표가 다른 것 6개 (E03, E09, E12, E13, E15, E19), 둘 중 하나라도 다른 것 10개. G3는 박자·조표를 보존하므로(설계) 두 판본 모두 고칠 수 없고, `bar_offset 0` 가정 아래 이 발췌들의 A·B는 참조와 다른 마디를 보여 준다. 참조와의 비교와 `nq.ned`에만 영향이 있고, A 대 B 판정은 같은 마디라 유효하다. 진 발췌는 박자가 맞는 쪽(E11, E15, E17)과 다른 쪽(E07, E12, E13)에 고루 있다.

### 30.3 발췌별 결과

"나음/나쁨"은 **G3a(B)가 G3 off(A)보다**. give·점수는 A(G3 off)/B(G3a)/C(참조). 쉼표 수는 각 파일의 `<rest>` (괄호 = G3a가 쓴 셋잇단 쉼표; G3 off는 20개 모두 0).

| 발췌 | 층 | 참조 (마디) | X·Y·Z | 리듬 | 성부/손 | 철자 | 전체 | give A/B/C | 점수 A/B/C | 쉼표 A→B (셋잇단 쉼표 B) | 박자·조 ≠ 참조 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| E01 | simple-duple | hanon/001 (1–8) | CAB | 같음 | 같음 | 같음 | **나음** | no/fix/yes | 2/3/5 | 64→68 | 박자 |
| E02 | simple-triple | sonatina/002 (1–8) | CBA | 같음 | 같음 | 같음 | 같음 | no/no/yes | 2/2/4 | 24→25 | 박자 |
| E03 | compound | czerny849/013 (5–12) | ACB | 같음 | **나음** | 같음 | **나음** | no/no/fix | 3/2/4 | 41→41 (5) | 박자, 조 |
| E04 | chromatic | czerny849/020 (9–16) | CBA | 같음 | 같음 | 같음 | 같음 | no/no/no | 3/2/3 | 46→45 (2) |  |
| E05 | two-voice | burgmuller25/013 (5–12) | CAB | 같음 | 같음 | **나쁨** | 같음 | fix/fix/fix | 3/3/3 | 48→56 |  |
| E06 | tuplets | burgmuller25/021 (17–24) | ABC | 같음 | 같음 | 같음 | 같음 | fix/fix/fix | 3/2/4 | 66→53 (5) |  |
| E07 | simple-duple | sonatina/009 (9–16) | ACB | **나쁨** | 같음 | 같음 | 같음 | fix/no/fix | 3/3/4 | 38→39 | 박자 |
| E08 | compound | czerny849/011 (1–8) | ACB | 같음 | 같음 | 같음 | 같음 | no/no/no | 1/3/2 | 45→50 (12) |  |
| E09 | two-voice | czerny849/007 (5–12) | CAB | 같음 | 같음 | 같음 | 같음 | fix/fix/fix | 3/3/3 | 71→83 | 조 |
| E10 | compound | burgmuller25/003 (9–16) | BAC | 같음 | **나음** | **나쁨** | 같음 | fix/fix/fix | 3/3/4 | 24→24 |  |
| E11 | simple-triple | sonatina/010 (61–68) | ABC | **나쁨** | 같음 | 같음 | **나쁨** | no/fix/fix | 2/3/4 | 26→33 |  |
| E12 | compound | burgmuller25/015 (29–36) | ABC | 같음 | 같음 | 같음 | **나쁨** | fix/fix/yes | 2/3/4 | 45→57 | 박자, 조 |
| E13 | tuplets | sonatina/012 (17–24) | CBA | 같음 | 같음 | 같음 | **나쁨** | no/no/yes | 3/2/4 | 71→77 | 박자, 조 |
| E14 | simple-triple | catalog/happy-birthday (1–8) | CBA | 같음 | 같음 | 같음 | 같음 | fix/fix/fix | 3/3/3 | 6→9 |  |
| E15 | chromatic | sonatina/018 (21–28) | ABC | **나쁨** | 같음 | 같음 | 같음 | fix/fix/fix | 3/3/4 | 44→50 | 조 |
| E16 | tuplets | czerny849/002 (5–12) | BAC | 같음 | 같음 | 같음 | 같음 | no/no/fix | 2/2/4 | 79→91 | 박자 |
| E17 | simple-duple | czerny849/001 (13–20) | BCA | **나쁨** | **나쁨** | 같음 | **나쁨** | fix/no/no | 3/2/3 | 67→63 (16) |  |
| E18 | simple-triple | catalog/gymnopedie-1 (1–8) | CAB | 같음 | 같음 | 같음 | 같음 | yes/yes/yes | 4/4/4 | 5→5 |  |
| E19 | two-voice | sonatina/017 (49–56) | ABC | 같음 | 같음 | 같음 | 같음 | fix/fix/no | 3/2/3 | 31→31 | 조 |
| E20 | simple-duple | burgmuller25/006 (9–16) | CAB | 같음 | 같음 | 같음 | 같음 | fix/fix/yes | 3/4/4 | 33→39 |  |
...
| 판본 | yes | fix | no | yes+fix | 점수 평균 | 점수 1–5 분포 |
| --- | --- | --- | --- | --- | --- | --- |
| A G3 off | 1 | 11 | 8 | 12 | 2.70 | 1, 5, 13, 1, 0 |
| B G3a | 1 | 11 | 8 | 12 | 2.70 | 0, 8, 10, 2, 0 |
| C 참조 | 6 | 10 | 4 | 16 | 3.65 | 0, 1, 6, 12, 1 |

- 사람에게 G3a는 G3 off와 **거의 구별되지 않는다**: 전체 14·리듬 16이 같음, 절대 평가가 칸마다 같다. 발췌별 점수 B−A: 오름 5, 같음 9, 내림 6, 평균 0. 층별로 몰린 곳은 없다 (패배: simple-duple 2, simple-triple 1, compound 1, tuplets 1).
- 최악: **E17** (Czerny 849/1, 셋잇단 연습곡) — 리듬·성부·전체 모두 패. E11·E12·E13 전체 패, E07·E15 리듬 패.
- G3a가 이긴 곳: E01 (Hanon 1, 전체 — 42 음이 staff를 옮김), E03 (Czerny 849/13, 성부·전체), E10 (성부).
- 메모에서 가장 많은 말은 **쉼표**다: "쉼표 이상함" 류가 7 발췌 (E02, E03, E07, E10, E11, E12, E14), 대부분 녹음 경로의 두 판본 모두에 대해. G3a(Y)에 대해서만 적은 것: E11 "쉼표 이상하고 쉼표 박자가 너무 이상해".

### 30.4 실패 유형 (판정을 움직인 것, 큰 순서)

| # | 등급 | 유형 | 증거 |
| --- | --- | --- | --- |
| **H1** | **BLOCKER** | **G3a가 이른 release로 짧게 적힌 음 사이의 틈을 더 많은 쉼표로 드러낸다.** 음가는 녹음 경로가 적은 그대로(이른 release; 고치는 것은 G3b R-reg, D1로 M11 전 OFF)인데, G3a의 구조 결정이 그 틈을 쉼표로 바꾼다: (a) 손 이동이 원래 staff에 박을 비워 쉼표를 만들고, 옮겨 간 짧은 음은 받는 staff의 긴 쉼표를 쪼갠다; (b) 박 단위 쉼표 분할 (4/4 `Rh.` → `Rq Rh`, 3/4 `Rh` → `Rq Rq`) — 참조와 같은 관례지만 기호가 는다; (c) 새 성부의 채움 쉼표 | 쉼표가 는 발췌 13/20 (합계 874 → 939, +7 %). 진 6 발췌는 모두 쉼표가 늘었거나(E07 +1, E11 +7, E12 +12, E13 +6, E15 +6) G3a만의 셋잇단 쉼표가 생겼다(E17 16). E11: 오른손 `q 8 8` → `q Rq` (8분 둘이 왼손으로 — 참조와 같은 손), 참조는 그 자리에 `h`를 끈다. E12: 8 음 이동, 왼손 쉼표 18 → 27. E15: `Rh.` → `Rq Rh` (참조와 같음) + 8마디 새 성부 `q Rq Rh`. **한계**: 쉼표 증가만으로는 선호를 잘 예측하지 못한다 (Kendall τ 0.18 전체, −0.03 리듬; 쉼표가 는 13 중 8은 리듬·전체에서 지지 않았고 그중 E01은 전체에서 이겼다) — 진 발췌의 공통 요인이지 유일 원인으로 증명된 것은 아니다 |
| **H2** | **MAJOR** | **G3a만 쓰는 셋잇단 안의 쉼표·점 음가** — 셋잇단 박 안의 release 틈을 time-modification 있는 쉼표·점 조각으로 정확히 적는다 (`16.`(3) + `32` 쉼표(3), `q`(3) 쉼표, `8`(3) 쉼표) | G3a 셋잇단 쉼표 40 (E17 16, E08 12, E03 5, E06 5, E04 2), 셋잇단 안 점 음가 10 (E03 4, E06 3, E17 3); G3 off는 둘 다 0. 참조 E17은 같은 자리가 고른 셋잇단 8분. E17은 리듬·성부·전체 패 (E03은 이김, E06·E08은 같음) |
| **H3** | **MAJOR** | **교사가 알아챌 이득이 없다** (기준 3) — 두 판본 공통의 결함(release 쉼표; 박자 7·조 6 발췌의 녹음 경로 오류)을 G3a가 건드리지 않고, G3a의 이득 중 하나(beam)는 이 렌더러에서 보이지 않았다 | yes+fix 12 = 12, 점수 평균 2.70 = 2.70, 참조 16·3.65. "쉼표 이상함"은 A·B 공통 |

MINOR: 철자 2 패 (E05, E10 — 메모 없음), 성부 1 패 (E17, H2와 같은 발췌).

### 30.5 제안하는 Fixer 작업 (이것만)

1. **H1 (P-staff·P-voice)**: 손·성부 이동은 떠나는 staff에도 받는 staff에도 **새 쉼표 시간을 만들지 않는다** — 이동 전에 음이 울리던(또는 한 쉼표이던) 자리가 이동 뒤 쉼표(또는 더 많은 쉼표)가 되면 critic 구성요소가 되돌리거나 손 DP 비용에 넣는다. 박 단위 쉼표 분할은 관례(참조와 같음)라 유지 — 바꾸려면 사용자 결정. fixture: E11 5–7, E12, E15 4·8, E07 3–5마디. 합격: 세트 20 발췌 쉼표 합계가 G3 off(874) 이하, core 마디당 쉼표가 G3 off 이하, G0·G3 gate 퇴행 0, `mutation-check` 전부 PASS.
2. **H2 (R-repr·T)**: 셋잇단 격자 한 칸보다 짧은 release 잔여(gate의 `R17_WIDE` 부류)를 쉼표·점 조각으로 적지 않고 writer가 적은 그대로 둔다 (R17처럼 R-reg로 미룸). 격자 한 칸 전체가 비어 있는 진짜 셋잇단 쉼표는 그대로. fixture: E17. 합격: 세트의 G3a 셋잇단 안 점 음가 0, 셋잇단 쉼표는 한 칸짜리만, reason-aware gate 결과 유지.
3. 그 뒤 **새 seed로** 세트를 다시 뽑는다 (`human-set --build`의 `SEED`) — 이 평가로 발췌별 X·Y·Z 뜻이 알려졌으므로 같은 seed의 재평가는 블라인드가 아니다. 그리고 A36을 다시 (사용자).

재평가 전에 필요한 사용자 결정 (Fixer가 정하지 않는다):

- **렌더러**: D7(a) MuseScore 4로 돌아갈지, 앱 렌더러를 계속 쓸지 (그러면 G3a의 beam·괄호는 판정되지 않음을 결정으로 기록).
- **D1 재검토 여부**: 가장 많은 사람 불만(release 쉼표)은 G3a가 아니라 R-reg(G3b)의 일이다. (a) H1·H2 수정 뒤 G3a만 다시 평가 (b) M11 실제 녹음 3곡으로 R-reg를 먼저 풀고 G3a+G3b를 함께 평가 (§21.5의 D).
- **세트 구성**: 녹음 경로의 박자·조가 참조와 다른 10 발췌를 세트에 둘지 — 결과를 본 뒤 빼는 것은 판정을 기울이므로, 바꾸려면 재평가 전에 결정으로 기록한다.
