...
- **G4f-2 flip RUNNING** — implementer on branch `g4f2-flip` in `D:/PPP-g4` (brief: scratchpad `brief_g4f2_flip.md`). Default `PPP.renderer` → `'engrave'`, `'legacy'` stays one switch away (A45); full review cycle because it changes the app file and what every user sees.

## 15. Next task

**G4f-2 — the flip (implementer running), then deploy on the user's word.** M-H2 accepted and the flip approved by the user (G4-U6, G04 §42.3).

Flip scope (brief `brief_g4f2_flip.md`): the default `PPP.renderer` becomes `'engrave'`; `?renderer=legacy` / localStorage `ppp.renderer = 'legacy'` become the opt-out (the §25 rollback is one line plus a deploy); A45 (legacy byte-identical under `'legacy'`), A30 (the eight browser suites) under both renderers; the print command becomes visible by default (it was gated on the engraver); the G4d-2 "decide before the flip" items (§37 G4f list: routed reduced views, first-draw long task) settled and recorded; tests that pinned "the default page loads nothing new" changed with reasons. No deploy, no removal of the legacy renderer (that is one release later, §25 step 3). Then one independent review, the Lead's re-check, merge, and **ask the user before `render deploys create`**.

Not gating the flip, kept for polish afterwards: the isolated-16th-note flag shape (§38.1, §42.1) and a possible narrow weak spot in dense sonatina passages (legacy won "overall" on 2 of that stratum's 4 excerpts, §42.2).
...
1. G4f-2 (flip, then deploy on the user's word)
