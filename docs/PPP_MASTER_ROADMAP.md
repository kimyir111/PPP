...
- **G4 polish m2 CLOSED (2026-09-27)** — PR #35, squash `7ecfb53`. Not a `scoregraph/` bug as first framed: `catalog/build-shared-seeds.js` wrote the non-standard `<stave-count>` instead of MusicXML's real `<staves>` tag, so the app's own parser never saw those 4 songs as two-staff and marked all of staff 2's hand `'x'` — `legacy.fromScore` was correctly refusing to reproduce a value no normal 2-staff piece can have. One-tag fix, regenerated `catalog/shared-seeds.json` (verified note-by-note: only `hand` and stale pre-MX-1 pitch-bookkeeping fields changed, nothing musical). Self-reviewed (tooling + seed data, not the app file or the engine, per review-depth-by-risk) and merged on a green gate. **Then a separate production step**, since `seedSharedScores()` skips ids that already exist: backed up the 4 live `ppp_shares` rows, dry-run confirmed only `hand` would change, the user confirmed the write, migrated, and verified live (all 7 shared seeds now draw with the engraver and show Print). **All 7 of 7 Shared Scores library songs now draw with the engraver in production.**
- **Three more polish items ran in parallel (user-requested), separate worktrees each — flag shape and sonatina beaming CLOSED with no code change, B5 investigated and deferred as backlog.** Full detail and reasoning in §15 item list below and G04 §46/§48/§49. A fourth, more serious finding (the windowed-view time-signature regression, G04 §47) surfaced during the flag-shape re-investigation and is being fixed now (still running) rather than deferred, since it's a real, live production bug on the app's default practice view.
...
**G4 polish, then §25 step 3.** No more human gates or approvals needed for these — ordinary engineering. m2 is done; its production migration also removed the one known reason A47/§25 step 3 couldn't yet claim 0 real-world fallbacks from the shared library. Three of the four polish items ran in parallel, each in its own worktree (the user asked for this explicitly), and are now resolved:

1. ~~The isolated-16th-note flag shape~~ — **CLOSED, no code change** (G04 §46). Re-investigated directly: the pinned font's real glyph, drawn per the graph's actual beam data (G4-D3), standard stem length. Not a defect; both M-H1 and M-H2 already said so.
2. ~~Dense/sonatina beaming (H03)~~ — **CLOSED, no code change** (G04 §48, PR #38 `d218505`). Every engrave-side complaint traced to engrave drawing the source MusicXML *more* completely than legacy (2 dropped slurs, 12 missing tenuto marks); every legacy-side complaint traced to a real legacy-only defect (a doubled-curve artifact, an unsubdivided 6/8 beam). A stratum-wide `legacy-geometry.js` run found no engrave weakness in dense passages — the opposite.
3. ~~B5's first-draw long task~~ — **INVESTIGATED, DEFERRED** (G04 §49, PR #39 `383a546`). Root-caused to three things, one of them (`scoreFromFile`/`resolve()`'s synchronous cost, ~125ms alone) outside `engrave/`'s scope entirely — no chunking inside the engraving engine alone reaches "0 long tasks." Fully closing it needs a Web Worker split of the whole pipeline (1–2+ weeks, the option that fully guarantees it) or a smaller partial fix that leaves gaps. This is exactly the catalog-scale cost the user asked to defer for now — left as backlog, not attempted.
4. **The windowed-view time-signature bug — RUNNING** (G04 §47, worktree `D:/PPP-g4`, branch `g4-polish-timesig`). A real, already-deployed regression, not cosmetic: the app's *default* practice view ("this part," not whole-score) can show no time signature at all once the practiced section isn't near the piece's start, because `engrave/layout.js`'s `showTime` only fires at the piece's true first measure or a real meter change, with no notion of "this window has no prior context" (confirmed directly: same code path, same bars, legacy shows "6/8," engrave shows nothing). The user chose to fix this now rather than defer it. Full implementer → independent review → Lead re-check cycle, since it's live screen-rendering engine code.
5. **§25 step 3, one release after the flip:** remove the legacy renderer and `loadVexFlow`'s CDN path, with the fallback counter at 0 as evidence — the shared-library reason is now closed; check A47's remaining 2 known-pathological fixtures before treating the counter as 0 in practice. Waits on item 4.
...
1. The time-signature fix (running), then §25 step 3 (flag shape/sonatina/B5/m2 all resolved)
