...
- **G4f-2 RUNNING (the Lead)** — M-H2 packet built, published, rated, and decoded (G04 §41–§42). **Result: borderline, one criterion technically fails.** "Overall" axis G4 ≥ legacy on 14/16 (passes, exactly at the line); G4's yes+fix ties legacy's 14/16 (passes); but 1 excerpt (H01) has "looks wrong" on engrave only, where §22.4 requires zero. Investigated directly (G04 §42.1): H01 is the *same file and bars* as M-H1's H03 (a different seed happened to pick the same sonatina/024, bars 100–107) and the *same already-understood* isolated-16th-note flag-shape question from M-H1 §38.1 — not a new or different defect, and arguably engrave's rendering is the more correct one (legacy visually fuses two different rhythms into one slanted beam). It was flagged for cosmetic polish before G4e and never actually touched. **Awaiting a user decision** on how to treat this one exception (§42.2) — this is a quality-bar call, not something the Lead will decide alone.

## 15. Next task

**G4f-2 (the Lead) — M-H2 result needs a user decision before the flip.** Depends on G4f-1 (closed) and MX-1 (already merged, satisfying G4f's precondition).

M-H2 full results and the H01 investigation are in G04 §42–§42.2. Two of three pass criteria clearly pass (engrave preferred 7/16, legacy 2/16, tied 7; legacy's own defects surfaced on 3 other excerpts, arguably worse than the one engrave nit). The one literal miss is a single, previously-known, cosmetic-only flag-shape question, not a new defect. The user is choosing between: **(A)** a small patch to the isolated-16th-note flag shape, then a fresh (third) blind round to get an unambiguous pass, or **(B)** accept this one exception as already-understood and not a real defect, and proceed to the flip on the current result. Also worth a look, not gating: a possible real, narrow weak spot in dense/sonatina passages (legacy won "overall" on 2 of that stratum's 4 excerpts — the only stratum where that happened).

If R5 (a nightly CI job with Chrome running the page tools and every browser suite separately) or A30-under-both-renderers isn't already fully covered by G4f-1's nightly wiring, close that gap before or alongside finishing G4f-2. Once the H01 question is resolved one way or the other, the flip (screen renderer defaults to `'engrave'`) still needs the user's explicit, separate sign-off — an irreversible, user-facing change, not just a passing gate.
