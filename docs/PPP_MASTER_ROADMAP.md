...
**G4 polish, then §25 step 3.** No more human gates or approvals needed for these — ordinary engineering. m2 is done (above); its production migration also removed the one known reason A47/§25 step 3 couldn't yet claim 0 real-world fallbacks from the shared library.

1. **The isolated-16th-note flag shape** (§38.1, §42.1) — cosmetic, low risk.
2. **A look at dense/sonatina beaming** (§42.2) — legacy won "overall" on 2 of that stratum's 4 M-H2 excerpts; worth understanding even though it didn't block the flip.
3. **B5's first-draw long task** (§16.3 time slicing) — pre-existing since G4d-2, unresolved by the flip.
4. **§25 step 3, one release after the flip:** remove the legacy renderer and `loadVexFlow`'s CDN path, with the fallback counter at 0 as evidence — the shared-library reason is now closed; check A47's remaining 2 known-pathological fixtures before treating the counter as 0 in practice.
...
1. G4 polish (flag shape, sonatina beaming, B5 — m2 done), then §25 step 3
