...
user decisions accepted (G04 §31), and **stage G4a implemented (G04 §32) and fixed after a parallel review (G04 §32.12,
2026-09-25): BLOCKER 0, MAJOR 0, ready for one more independent review** — not merged. G4a changes nothing a user
...
- **Active goal: G4 Professional Engraving — G4a implemented and fixed (BLOCKER 0, MAJOR 0), READY_FOR_REVIEW.**
  ...
  user's conditions), the G4a record §32 and its fixer record §32.12 (DECISIONS G4-F1–F14). **Next: one more independent
  review of G4a** (§32.9 and §32.12.1), then G4b (layout core, the renderer switch, the practice map in the
  page). What G4b inherits from the fixer (§32.12.10): the vendored VexFlow is served `no-store` without gzip; a reloaded
