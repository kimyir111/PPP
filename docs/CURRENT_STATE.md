...
attached to systems, vertical spacing, courtesy signs) was reviewed (NEEDS_FIX, MAJOR 3) and its Fixer is **READY_FOR_RECHECK**
on branch `g4d1b-system-marks` (G04 §36, Fixer §36.18; DECISIONS G4-D1b-1–24), not merged. Read this first in a new session, then
...
| G4 | **G4d-1a CLOSED — merged as PR #17 (`b4fe019`)** (G04 §35, review and Fixer §35.18, re-check and merge §35.19; DECISIONS G4-D1a-1–23, G4-L4, G4-L5). Ties (halves at a system break, chord ties split relative to the chord, each end nearest its own head), slurs (graph pairs, clearing notes, marks and fingering; one part per system), glissandi, articulations, ornaments, fermatas, tremolo, fingering by its notes inside slurs, arpeggios, noteheads, cautionary accidentals, percussion heads; one `place()` over skylines with `FAR_PLACEMENT`; text widths from the pinned fonts' table (`engrave/metrics-text.js`); `plan/2`, `engr/3`, and a version guard. Earlier: G4a PR #9, G4b PR #12, G4c PR #15. Node only: nothing a user sees changed. **G4d-1b FIX: READY_FOR_RECHECK** (branch `g4d1b-system-marks`, G04 §36, review and Fixer §36.18, DECISIONS G4-D1b-1–24): lyrics, dynamics and hairpins between a grand staff's staves on one line a system, pedal (a change drawn as a change), octave lines over exactly their notes with "(8)" after a break, chord names, voltas, tempo, rehearsal marks, jumps, words — through the one `place()`; §15.3 spacing with the rows between the staves centred, §15.4 courtesy checked at every system end, bracketed accidentals (R5); the Fixer puts the words a file prints around a metronome mark on its line (review R1) and holds A8's positions, hairpin extents and G4-D1b-4/-5 by named zero-target metrics (R2, R3); `plan/3`, `engr/5`, 34 new live mutations. Node only. Then G4d-2 + M-H1. |
...
- **Active goal: G4 Professional Engraving — G4a, G4b, G4c and G4d-1a CLOSED (PR #9 `df8a571`, PR #12 `62ede61`, PR #15
  `e3c8c5a`, PR #17 `b4fe019`); G4d-1b FIX: READY_FOR_RECHECK; next the Lead's re-check, then G4d-2.** The order, gates and briefs are in `docs/PPP_MASTER_ROADMAP.md` (§14 current task, §15 next).
  ...
  - **G4d-1b — FIX: READY_FOR_RECHECK** — worktree `D:/PPP-g4`, branch `g4d1b-system-marks` (code `17a38f3`, Fixer `3746437`, pushed, no PR):
    G04 §10.2 rows 7–11 (lyrics, dynamics, hairpins, pedal, ottava, volta, chord names, tempo, rehearsal, jumps, words),
    §15.3 vertical spacing, §15.4 courtesy signs, B9 re-measured (0.32), the G4d-1a review's R5 (G04 §36). The independent
    review returned NEEDS_FIX (MAJOR 3: tempo words split around a metronome mark, A8's positions and two new rules held by the
    layout hash alone); the Fixer's record is G04 §36.18 (M2, a tempo word in its own direction stacked over the mark, is on
    the M-H1 watch list). E17 and E18 are unchanged (MX-1's tests hold them), the break cases are in E37.
