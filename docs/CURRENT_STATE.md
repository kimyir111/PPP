...
sees. **G4d-1a** (curves and marks attached to notes; G4-L3) is implemented on branch `g4d1a-curves-marks` and waits for its
independent review (G04 §35). Read this first in a new session, then
...
| G4 | **G4d-1a READY_FOR_REVIEW** on `g4d1a-curves-marks` (G04 §35, DECISIONS G4-D1a-1–13): ties (halves at a system break, inferred ties drawn), slurs on the graph's pairs, glissandi, articulations, ornaments, fermatas, tremolos, fingering, arpeggios, notehead shapes and enclosed accidentals, all through one placement function; the §18.3 text-metrics table (`engrave/metrics-text.js`, `make-text-metrics.js --check --fetch` in CI); ledger lines for rests off the staff; tuplet numbers by their beams; `plan/2`, `engr/2` and the rule that every output change moves the version. Node only, nothing a user sees changed. **G4c CLOSED — merged as PR #15 (`e3c8c5a`)** (G04 §34, review and Fixer §34.18, re-check and merge §34.19; DECISIONS G4-C1–C16, G4-L2, G4-L3). Graph beams drawn exactly and derived beams only where the part has none; final stems; tuplets with nesting and the one-note display; voices with shared unisons and the down-stem voice moved right at a second (G4-B11); rests clear of the other voice and centred whole-bar rests; grace notes at 0.66; the mid-measure key change; `svg.js` (0.19–0.28 of the legacy SVG size, B9). Rest and stem/flag collisions are zero-target gates (189 → 0, 216 → 0). Node only: the app does not load it, and nothing a user sees changed. **Next: G4d-1a** in `D:/PPP-g4` (curves and marks attached to notes), then G4d-1b, G4d-2 + M-H1. |
...
- **Active goal: G4 Professional Engraving — G4a, G4b and G4c CLOSED (PR #9 `df8a571`, PR #12 `62ede61`, PR #15
  ...
  - **G4d-1a — READY_FOR_REVIEW** — worktree `D:/PPP-g4`, branch `g4d1a-curves-marks` (pushed, no PR): G04 §13 ties,
    slurs and glissando, `place()` over skylines, the marks attached to notes, the §18.3 text-metrics table, and the G4c
    review's note-level carry-overs (G04 §35). Next: its independent review, then G4d-1b (marks attached to systems).
