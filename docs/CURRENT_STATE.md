...
user decisions accepted (G04 §31), and **stage G4a implemented (G04 §32, 2026-09-25) and ready for one independent
review** — not merged. G4a changes nothing a user sees: the legacy renderer is still the one drawing. Read this first
...
- **Active goal: G4 Professional Engraving — G4a implemented, READY_FOR_REVIEW.** Worktree `D:/PPP-g4`, branch
  `g4-professional-engraving`, based on `55d1bd5`. Spec `docs/GOALS/G04_PROFESSIONAL_ENGRAVING.md`: design §0–§30, user
  decisions §31 (G4-U1–U4 accepted, U2 and U4 with the user's conditions), the G4a record §32. **Next: one independent
  review of G4a** (§32.9 lists what to look at), then G4b (layout core, the renderer switch, the practice map in the
  page). Production keeps the legacy renderer until G4f. What G3 left for G4 is in G03 §30–§31 and DECISIONS G3-D3,
  G3-D4 and G3-U10.
