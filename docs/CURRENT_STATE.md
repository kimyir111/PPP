...
Updated 2026-09-25 (G4a, on `g4-professional-engraving`; `main` as below). G0 (Quality Foundation), G1 (ScoreGraph) and G2 (Score Import) are all merged
...
and the pedal join — and nothing a user sees changed. `origin/main` is `c5474c2` plus the docs-only closeout that
records it. **The active goal is G4 Professional Engraving** (worktree `D:/PPP-g4`, branch
`g4-professional-engraving`): designed (`docs/GOALS/G04_PROFESSIONAL_ENGRAVING.md`, Architect 2026-09-24), the four
user decisions accepted (G04 §31, §32.13 G4-U5), and **stage G4a implemented (G04 §32), fixed after a parallel review
(§32.12) and finalized after its final independent review (§32.13, 2026-09-25): BLOCKER 0, MAJOR 0,
READY_FOR_FINAL_REVIEW** — not merged. G4a changes nothing a user
sees: the legacy renderer is still the one drawing (16/16 renders byte-identical to `55d1bd5`). Read this first
in a new session, then the current goal's spec in `docs/GOALS/`.
...
| `main` | **Local `main` is `d82bb71`, which must not be pushed.** Despite its message ("harden G0 quality benchmark") it holds no benchmark code: it is a `git add -A` sweep of `D:/PPP` with copyrighted `tmp/` audio and score renders, `__pycache__`, a `_oh-sheet-compare` gitlink and another session's 124 `catalog/method` files (G00 §19.18). It is not pushed, and `origin/main` (`c5474c2`) does not contain it. It is unrelated stale local state: **leave `D:/PPP` and `d82bb71` untouched**; the user decides how to undo it. |
...
- **Active goal: G4 Professional Engraving — G4a finalized after its final review (BLOCKER 0, MAJOR 0), READY_FOR_FINAL_REVIEW.**
  Worktree `D:/PPP-g4`, branch `g4-professional-engraving`, based on `55d1bd5`. Spec
  `docs/GOALS/G04_PROFESSIONAL_ENGRAVING.md`: design §0–§30, user decisions §31 (G4-U1–U4 accepted, U2 and U4 with the
  user's conditions; G4-U5 the deferred contract), the G4a record §32, its fixer record §32.12 and its final record §32.13
  (DECISIONS G4-F1–F21). **Next: the final review of G4a** (§32.13.1 lists what closed each finding), then G4b (layout core,
  the renderer switch, the practice map in the page). What G4b inherits from the fixer (§32.12.10): the vendored VexFlow is served `no-store` without gzip; a reloaded
  song's `resolve` costs up to 316 ms at 4× CPU; the plan writes 8va notes at concert − shift (§8.3), an octave from
  where the legacy renderer draws them (issue 3 — a user decision before the flip). Production keeps the legacy renderer until G4f. What G3 left for G4 is in G03 §30–§31 and DECISIONS G3-D3,
  G3-D4 and G3-U10.
