...
Updated 2026-09-25 (G4a merged as PR #9; G4b implemented on `g4b-layout-core`, not merged). G0 (Quality Foundation), G1 (ScoreGraph) and G2 (Score Import) are all merged
...
renderer is still the one drawing (16/16 renders byte-identical to `55d1bd5`). **G4b (layout core) is implemented and
waiting for its review** (G04 §33, DECISIONS G4-B1–B10): worktree `D:/PPP-g4`, branch `g4b-layout-core` (from `a0bc2ea`,
pushed to `origin/g4b-layout-core`; no PR, not merged). It lays a NotationPlan out as an EngravedScore in Node and in the
browser alike and changes nothing a user sees — the app does not load it. Read this first in a new session, then the
...
| G4 | **G4b implemented, reviewed and fixed — READY_FOR_MERGE_CHECK** (branch `g4b-layout-core`, G04 §33, fixer §33.16; not merged): the six G4a MINORs closed first (store timeout, A48 population gate with negative controls, sibling tuplets, meter beat groups, tuplet display in the ledger signature, deferred code × kind); then the layout core in `engrave/` — `metrics.js` (glyph boxes generated from the vendored Bravura, `make-metrics.js --check`), `space.js` (spring–rod, exact solve), `breaks.js` (G4-U4 line breaking: 4/2 bars preferred, density wins), `skyline.js` (skylines, placement, hard-collision checks), `canon.js` (canonical JSON + FNV-1a 64 layout hash), `layout.js` (NotationPlan → EngravedScore `engr/1`: notes, rests, accidentals, dots, ledgers, provisional stems/flags, clef/key/time, bar lines, repeats, voltas, braced grand staves; a system too wide for a phone is drawn at a smaller staff size), `practice.js` (PracticeMap: ids → geometry, onset keys, hit-test, seek, loops, the legacy `_map` shape; a highlighter that touches only what changes). Node = headless Chrome on 808/808 layout hashes; every G04 time budget G4b can judge is met (sonatina/020 whole layout 24 ms; B5's 12 ms chunks need G4f's time slicing). The independent review's three MAJORs are closed (fixer, §33.16): an A29 static ban that can fail (41 negative controls), 12 committed layout mutations each caught by a named metric, and other-voice collisions measured as ratchet gates (R suite: rest 34, stem/flag over a head 74; G4c takes them to 0). The app does not load any of it (G4-B1); SVG, the renderer switch, beams and the rest are G4c–G4f. **G4a CLOSED — merged as PR #9 (`df8a571`)** after its final independent review (BLOCKER 0, MAJOR 0; G04 §32 implementation, §32.12 fixer, §32.13 final, §32.14 closeout; the branch `g4-professional-engraving` is kept). G4a: `engrave/` (render source by content, NotationPlan with a fidelity ledger audited against the plan's **output**, glyph table, an IndexedDB v2 graph cache kept beside each song), `legacy.fromScore`/`agree`/`link` in `scoregraph/legacy-score.js` (library 1.3.0; chords compared at the chord), VexFlow 4.2.3 vendored in `vendor/` with every font notice, E01–E40 fixtures, the R corpus manifest (61 files, no quarantine, no G0 hold-out), the plan-level L1 benchmark (`tests/engrave/tools/bench.js`, suites r/e/x) and `npm run test:engrave` in CI. Measured (§32.12, §32.13): A48 over every committed import file as the app holds it (544: 540 exact, 4 named losses) and the 553 core transcriptions (exact live, projected, saved, migrated); every user path, the review screen's rewrites, arrangements and Accept included, keeps its graph through an immediate reload; 57/57 source mutations of the plan, ledger, store and `fromScore` caught; persist has no long task at 4× CPU. The legacy renderer is byte for byte unchanged and still draws everything; G3 stays off. Design (G04, 2026-09-24) below. No schema or golden changed. The audit found the renderer never reads the ScoreGraph: `legacy-score.js` `toScore` drops every beam (11,808 beams in the 357 committed scores), grace notes, most articulations and the tuplet display options; the renderer draws no dynamics, hairpins, articulations or printed fingering, invents its own beams and tuplet marks, hides inferred ties within a bar, and drops ties and slurs across a line break. Recommended: keep VexFlow 4.2.3 (vendored) under a PPP engraving layer — ScoreGraph → NotationPlan (with a fidelity ledger) → EngravedScore → SVG, with the same layout code in Node (CI benchmark) and the browser; `legacy.fromScore` so a saved song always has a graph; renderer switch `PPP.renderer` defaulting to `'legacy'` until G4f. Stages G4a–G4f (G04 §27). |
...
- **Active goal: G4 Professional Engraving — G4a CLOSED (PR #9, `df8a571`); next G4b, not started.**
  Worktree `D:/PPP-g4`, branch `g4b-layout-core` from the latest `origin/main` (no upstream set on purpose). Spec
  `docs/GOALS/G04_PROFESSIONAL_ENGRAVING.md`: design §0–§30, user decisions §31 (G4-U1–U5), the G4a records §32–§32.14
  (DECISIONS G4-F1–F21). **Next: G4b** (§27: layout core, the renderer switch `PPP.renderer` defaulting to `'legacy'`,
  the practice map in the page). Its backlog from G4a's final review (MINOR, G04 §32.14):
  ...
  What G4b also inherits (§32.12.10): the vendored VexFlow is served `no-store` without gzip; a reloaded song's
  `resolve` costs up to 316 ms at 4× CPU; the plan writes 8va notes at concert − shift (§8.3), an octave from where the
  legacy renderer draws them (issue 3 — a user decision before the flip). Production keeps the legacy renderer until
  G4f. What G3 left for G4 is in G03 §30–§31 and DECISIONS G3-D3, G3-D4 and G3-U10.
