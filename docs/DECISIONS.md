...
| G4-U6 | **M-H2 수용, flip 승인** (2026-09-26, G4f-2). M-H2는 "전체" 축 14/16(경계값)과 yes+fix 동점(14:14)을 통과하고, "engrave만 틀려 보임 0개"를 한 발췌(H01)로 어겼다. 그 발췌는 M-H1 §38.1에서 이미 조사해 결함이 아니라 관례 차이(홀로 낀 16분음표 flag — legacy는 서로 다른 리듬을 한 beam으로 이어 그린다)로 판정한 바로 그 파일·마디다(다른 seed가 우연히 다시 뽑음). Lead가 사실을 그대로 보이고 두 길(flag 다듬고 셋째 평가 / 지금 수용하고 flip)을 드렸고, **사용자가 "지금 결과로 통과 처리하고 flip 진행"을 골랐다.** A44 = PASS(예외 1개 수용), §25 2단계 flip 승인. 배포는 병합 뒤 따로 확인 | flag 모양을 다듬고 새 seed로 셋째 평가 (더 확실하지만, 이미 결함이 아니라고 판정된 자리 하나 때문에 몇 시간과 사용자 45분을 더 씀) | G04 §42–§42.3 |
...
| G4-L6 | **화면 페이지의 SVG는 inline이다** (`engrave/page.js` `SVG_OPTS` `inline: true`; G4f-2 리뷰 R1, 2026-09-26). 버린 대안 (리뷰가 4× 재생 rAF p95로 잰 것, 세 라운드, legacy 69.5–111.2 ms, `<use>` 104.3–208.4, inline 48.6–69.4): 앞 450개 glyph만 inline이고 나머지는 `<use>`인 혼합 62.7–111 (long task 3–30); 재생선·마디 wash를 따로 된 overlay 층으로 97.3–166.7 (30–39); overlay + glyph 미리 크기 맞춤 125–187.6 (36–43); `will-change` 층 분리 — 모두 `<use>`의 다시 기록 비용을 없애지 못함. B9(≤ 0.5× legacy)는 화면에 대해 "legacy 이하 (지금 사용자가 받는 것보다 크지 않음)"로 바꾼다: inline 0.51–0.78. `svg.js`의 기본값과 인쇄는 `<use>` | 매끄러운 재생이 제품이다. B9는 대리 예산이었고, inline도 오늘 사용자가 받는 legacy SVG보다 작다. G04 §43.10; DECISIONS G4-D2-21·22 정정 |
| **G4-L7 (제안, Implementer — Lead 재확인 대기)** | **창(window)의 첫 표시 마디가 곡의 진짜 첫 마디가 아니면, 거기서 박자표를 보여준다** — clef·key가 이미 그러듯이. 정확히: `layout.js`의 `layout(P, config)`가 계산하는 `lo`(`cfg.window`가 있으면 `Math.min(cfg.window[0], n-1)`, 없으면 0)가 0보다 크면, 그 창의 **첫 시스템에서만**(둘째 시스템부터는 아님) 박자표를 강제로 그린다. 창이 곡의 진짜 시작을 포함하면(`lo===0`)과 복습(review) 패킷을 특별 취급할 필요는 없다 — 후자도 그냥 `S.reviewFrom`이 만드는 창이 이 규칙을 그대로 타서 고쳐진다(§47.4). Print(창이 항상 `null`)는 이 규칙이 적용될 일이 없다 | flip(G4f-2, PR #32)의 회귀: 연습 화면(`Piano Coach App.dc.html` ~18384, `viewStart()`가 만드는 뷰 — 가장 흔한 뷰)이 곡 중간에서 시작하면 그 구간에 박자 변화가 없는 한 화면 어디에도 박자표가 없었다. §15.4(과 legacy)의 관례상 clef·key는 매 시스템 다시 그리고 time은 바뀔 때만이지만, "창의 강제된 시스템 시작"은 그 관례가 말하는 통상의 시스템 나눔이 아니라 학습자가 보는 화면의 진짜 시작이므로 legacy처럼 항상 다시 그려야 한다는 것이 이 결정의 취지. G04 §47 (구현·seam·mutation·회귀) |
...
| G4-D2-21 | ~~**B9 회복: 페이지의 SVG_OPTS를 `inline: false`로** (리뷰 R3): `engrave/page.js:74`의 `SVG_OPTS`가 `inline: true`(글리프마다 리터럴 path)에서 svg.js의 기본인 `inline: false`(글리프마다 `<defs>`의 `<symbol>` 하나 + `<use>`)로 — 새 코드를 더 쓰지 않고 이미 있던 svg.js의 두 모드 중 하나를 그대로 씀. 리뷰가 실측한 5개 파일(비-hold-out 실제 곡) 모두 B9(legacy의 0.5배 이하) 안쪽으로 회복: burgmuller25/021 0.575→0.238, czerny849/001 0.514→0.291, sonatina/013 0.567→0.257, sonatina/016 0.778→0.341, sonatina/020 0.787→0.356 — waiver 불필요~~ **정정 (G4f-2 리뷰 R1, Lead 결정 G4-L6, 2026-09-26): 화면 페이지의 SVG는 다시 `inline: true`다.** B9(≤ 0.5× legacy)는 대리 예산이었고, `<use>`의 대가는 재생이었다 — 재생 틱마다 악보 `<svg>` 전체의 paint를 다시 기록하는 비용이 `<use>`에서 훨씬 크다 (리뷰 trace, 4×: Paint 51.7 ms `<use>` / 8.5 inline / 17.2 legacy). 화면의 B9는 "legacy 이하"로: inline 0.51–0.78 (위 다섯 곡: 0.57, 0.51, 0.56, 0.77, 0.78). `svg.js`의 기본값과 인쇄는 `<use>` 그대로 (G04 §43.10) | B9을 그대로 어긴 채 waiver(§19.1 예외)로 남기기 — B9이 완전히 회복되고(전부 0.4 이하) 실측 프레임 시간에 유의미한 퇴행이 없어(G4-D2-22) 이 대안을 쓸 필요가 없었음 | 독립 리뷰 R3 (증거: `g4d2-review/b9.js`); G04 §37.18.4 |
| G4-D2-22 | ~~**repaint 배수의 기록 정정** (리뷰 R3, M2): §16.3·이전 DECISIONS가 적은 "Chrome이 `<use>`를 path보다 4–5배(때로 8–10배) 느리게 repaint"는 이 Fixer의 재측정(`repaint2.js`, sonatina/020 전곡 150프레임, Chrome trace)으로 다음과 같이 교정한다 — **paint류 trace 성분의 합은 실제로 약 4.6배**(`<use>` 10.03 ms/프레임 대 inline 2.18 ms/프레임, 독립 리뷰의 "약 2배"보다 큼)이지만, **체감 프레임 시간(중앙값)은 사실상 같다**(14.0 ms 대 13.9 ms) — paint류 작업이 16 ms 프레임 예산의 일부에 지나지 않아, 이론적 배수만큼 실제 재생이 느려지지 않는다. 기록은 "trace 성분 기준 4–5배가 맞고, 실제 재생 프레임 시간에는 (이 곡에서는) 측정 가능한 차이가 없다"로 남긴다~~ **정정 (G4f-2 리뷰 R1, 2026-09-26): "체감 프레임 시간은 같다"는 틀렸다.** 150 프레임의 중앙값만 봤고 p95·long task·CPU 감속과 실제 재생을 보지 않았다. 같은 곡(sonatina/020 전곡)을 번갈아 3라운드 잰 값: 1× `setState` 프레임 p95 `<use>` 19.0–23.0 ms / inline 7.6–10.0 / legacy 10.7–16.8, 4× 117.2–250.1 / 52.6–63.5 / 85.4–168.8 ms, 4× 재생 중 long task `<use>` 43–50개(최대 126–220 ms) / inline 2–49 / legacy 7–26 — `<use>`는 재생에서 legacy보다 느렸다 (G04 §43.8, §43.10.2) | 리뷰의 "약 2배"를 그대로 옮겨 적기(내 재측정과 다름 — 방법 차이일 수 있으나, 내가 직접 잰 숫자를 적음) | 이 Fixer의 재측정; G04 §37.18.4 |
| G4-D2-23 | **M-H1 packet의 두 번째 블라인드 누설을 내보내기 도구에서만 고친다** (Lead 재확인, 2026-09-26): R3의 B9 수정(G4-D2-21)이 페이지를 svg.js의 `<defs>`/`<symbol>` + `<use>` 모드로 되돌리면서, 판각기 쪽 SVG에만 `<use>`/`<symbol>` 태그가 남아 알고리즘 없이도 16/16 완벽하게 갈리는 새 지문이 생겼다. `tests/engrave/tools/review-build.js`의 내보내기 복사 단계에서만 각 `<use>`를 그 `<symbol>`의 리터럴 복제로 바꾸고 빈 `<defs>`·`<symbol>`을 지운다 — 앱의 실제 출력은 손대지 않아 B9는 그대로 회복된 채다. 시험용 packet(버림)에서 두 쪽 다 태그 0개, 파일 길이 겹침, 키 복원 우연 수준(6–11/16)을 확인했다 | 페이지 자체를 다시 `inline: true`로 되돌리기(B9를 다시 깬다) — 문제는 packet에 무엇이 나가는지이지 앱이 무엇을 그리는지가 아니다 | Lead 재확인; G04 §37.19 |

## G4e — 페이지와 인쇄 (Implementer, 2026-09-26; `docs/GOALS/G04_PROFESSIONAL_ENGRAVING.md` §39)

브랜치 `g4e-print` (`D:/PPP-g4`), 시작 `338508d`. 병합하지 않았고 PR도 없다.

| ID | 결정 | 버린 대안 | 근거 |
| --- | --- | --- | --- |
| G4-E1 | **인쇄 줄바꿈은 화면과 완전히 분리된 DP** (`breaks.js` `printBreakLines`/`printSystemCost`): 비용 `100·(stretch−1.15)²`, 1마디 +50(전곡이 1마디면 면제 — 별도 분기 없이 DP의 최소화 자체가 "피할 수 있을 때만 물린다"를 만족시킨다), 1–6마디, 마지막 system은 `stretch<1`(압축이 필요했을 상황)일 때만 비용 0. 화면 `breakLines`는 한 글자도 고치지 않았다 | 화면 DP를 매개변수화해서 재사용 — 뮤테이션 anchor(M7a·M7b)가 겹치는 문제가 실제로 생겼고(§39.7), 무엇보다 화면의 committed hash에 조금이라도 손댈 위험을 지는 이득이 없다 | G04 §15.5; §39.3 |
| G4-E2 | **여러 마디 쉼표는 굵은 막대 + 마디 수**, SMuFL의 여러 마디 쉼표 글리프(2–4마디와 5마디 이상의 hash-mark 변형)는 이번 세션에 새로 pinning하지 않는다. 병합은 `layoutPrint()`가 그때그때 만드는 마디 사본(`mergeRests`)에서만 하고 `prepare()`(화면·인쇄 공유)는 손대지 않는다 — 화면은 `mergeRests`를 아예 부르지 않는다(도달 불가, committed hash 무위험). 병합된 런은 `printBreakLines`의 새 `banned` 매개변수로 system 시작점이 되지 못하게 막아 절대 쪼개지지 않는다 | 실제 SMuFL 글리프 pinning(`outlines.js`에 새 글리프, `make-outlines.js` 갱신) — 범위 밖의 새 글꼴 작업, 이번 세션 시간 안에 위험 대비 이득이 낮음 | G04 §17.3, A11, A40; §39.4 |
| G4-E3 | **"1마디 벌점" mutation은 실제 코퍼스 파일로 못 걸어 `printSystemCost`(공개 함수) 직접 호출로 증명한다.** 60개 R 코퍼스+E fixture 스윕으로 벌점 제거가 실제 결과를 바꾸는 파일을 찾지 못했다(마디 하나가 시스템 하나만큼 넓어지려면 극단적인 폭 차이가 필요해, 정상적인 피아노 악보에서는 나타나지 않는다) | 인위적인 합성 ScoreGraph를 손으로 만들어 실제 엔진 전체를 통과시키기 — 시간 대비 이득이 낮고, 비용 함수 자체를 직접 부르는 쪽이 오히려 더 정확한 증명 | §39.3, §39.6; `print.test.js` |
| G4-E4 | **인쇄 페이지마다 독립 glyph-symbol id 접두사** (`page.js` `printSvgs`, `idPrefix: 'ppp-print-' + i + '-g-'`): 이 세션 자신의 puppeteer 확인(§27 G4e 카드의 "직접 봄")이 실제로 잡은 결함 — 기본 접두사 `ppp-g-`를 여러 인쇄 페이지와, 숨겨졌을 뿐 DOM에 남아 있는 화면 SVG(`unit 10`)가 공유해 `<use>`가 10배 큰 심볼을 가리켰다(페이지를 뒤덮는 검은 얼룩) | 인쇄 컨테이너를 iframe으로 격리(별도 문서, id 충돌 원천 차단) — 더 큰 변경이고, 접두사 하나로 충분히 고쳐지는 문제 | §39.10.1; `print.test.js`의 회귀 검사 |
| G4-E5 | **`engr/5` → `engr/6`, `plan/3` 유지.** 화면 출력은 바이트 그대로지만(layout-diff.js, 236/236 SERIALIZATION_ONLY (version)) `layout()`의 계약 자체(새 `page` 필드, 여러 페이지 `pages`)가 넓어졌으므로 G4-D1a-1의 규칙대로 올렸다. `plan.js`는 무변경이라 `PLAN_VERSION` 그대로 | 버전 유지(화면 출력이 안 바뀌었으므로) — 계약이 넓어진 것 자체가 "출력이 바뀜"에 해당한다고 읽음(선례: G4d-1b도 화면에 드러나지 않는 변화까지 올림) | G4-D1a-1; §39.7.1 |
| G4-E6 | **여러 part 이름은 첫 system만; 둘째 이후 system의 약어(§15.5 "약어는 그 밖의 system")는 이번 세션에 하지 않는다.** ledger는 `part-abbr`를 `drawn`으로 이미 표시하지만(plan.js, G4a부터), `ledger.audit()`은 plan 출력만 보고 layout이 실제로 그렸는지는 보지 않으므로 감사는 그대로 통과한다 | 매 system마다 약어 배치 구현 — 시간 대비 범위가 넓어(모든 system의 세로 위치 재계산), 브리프의 명시적 문구("첫 system에 part 이름")를 넘어서는 확장 | §39.1, §39.11; G04 §15.5 |
| G4-E7 | **§15.3의 60% 채움 세로 맞춤(페이지가 그만큼 차면 system 사이를 늘려 아래까지 맞춤)은 G4f로 미룬다.** 이 브리프의 명시적 Scope 목록에 없다(§15.5·§17.1만 열거). ~~지금은 항상 "위에 붙임" 쪽으로 동작한다 — 실제로 본 6곡 모두 첫 페이지가 92% 안팎까지 차 시각적으로 어색하지 않았다~~ **정정(Fixer, 2026-09-26, §39.12): 이 근거는 틀렸다.** R 코퍼스 전체(61개 파일, 96페이지)를 재서 보면 평균 채움은 **65.3%**이고, **35/96(36%)이 50% 미만, 51/96(53%)이 70% 미만**이며, 직접 본 6곡 중 하나인 Für Elise가 **18.8%**(제목 + 4마디 + 빈 여백)로 가장 낮다 — "대체로 위 절반만 어색하다"가 아니라 흔한 경우다. §15.3 미구현이라는 결정 자체는 바뀌지 않았다(이 사이클의 범위는 기록을 바로잡는 것이지 기능을 넣는 것이 아니다) | 이번 세션에 구현 — 페이지마다 남는 여백을 언제 "60% 이상"으로 볼지, 어떻게 고르게 늘릴지 설계가 더 필요해 시간 안에 안전하게 끝내기 어려움 | §39.1, §39.11, §39.12 |
| G4-E8 | **`layout-hashes.js`의 `CONFIGS`에 `print: {mode:'print'}`를 추가해, 화면 desktop·phone과 같은 매커니즘으로 인쇄 layout의 committed 회귀 baseline을 만든다.** 별도 파일이나 별도 도구를 새로 만들지 않는다 — `createEngraver(plan).layout({mode:'print'})`가 이미 화면 config와 똑같은 `EngravedScore` 모양(다만 `pages`가 있음)을 내므로, 기존 `computeAll`/`diff`/`guard`가 그대로 적용된다. R(61)+E(다수)+X(golden 17) 전체 스코프, 같은 버전 가드(같은 버전 아래 해시가 다르면 `--write` 거절)가 인쇄에도 그대로 적용된다. **가드의 빈틈 하나를 따로 메웠다**: merge base(`origin/main`)는 이 브랜치가 아직 병합되지 않아 `print` config를 전혀 모르므로, `diff()`만으로는 이 브랜치가 살아있는 동안 print 해시를 바꿔도 버전을 올리지 않고 조용히 `--write`가 통과했다(실측: 실제로 통과함, 아래 증거) — `layout-hashes.js`에 `diffAgainstNewConfigs()`를 추가해, merge base가 모르는 (score, config) 쌍은 대신 디스크에 이미 커밋된 파일과 같은 버전 아래 비교해 실패로 닫는다 | 인쇄 전용 baseline 파일이나 별도 `--check` 도구 신설 — 기존 메커니즘이 config 하나 늘리는 것만으로 완전히 커버해 별도 파일을 만들 이유가 없다(같은 버전 규율, 같은 실패 모드) | §39.12(R2) |

## G4f-1 — benchmark 완성: mutation, legacy 비교, 성능, CI (구현 2026-09-26; `docs/GOALS/G04_PROFESSIONAL_ENGRAVING.md` §40)

**ID 표기**: 이 절의 ID는 `G4-F1-N`이다(`G4-F1`이 아니라) — `G4-F1`…`G4-F21`은 이미 G4a Fixer 라운드(§32.12–32.13)가 쓰고 있어(그 절의 "F"는 "Fixer"의 F), 브리프가 준 "G4-F1…" 표기를 그대로 쓰면 기존 결정과 충돌한다. G4d-1a/1b가 `G4-D1a-N`/`G4-D1b-N`으로 하위 단계를 적은 것과 같은 모양으로 `G4-F1-N`(G4f **1**단계)을 쓴다.

| ID | 결정 | 버린 대안 | 근거 |
| --- | --- | --- | --- |
| G4-F1-1 | **M22는 `tests/engrave/layout-mutation.test.js`의 MUTATIONS 배열이 아니라 별도 `test()`로 둔다.** M22는 `engrave/practice.js`(sync)를 잡는데, 이 파일의 다른 mutation은 전부 `engrave/layout.js`류(레이아웃 산출물, EngravedScore)를 겨눠 `run()`/`l2()`로 재는 공용 틀을 쓴다 — sync는 EngravedScore를 전혀 만들지 않으므로 그 틀에 맞지 않는다. M25(인쇄, `print-l2.js`)가 이미 같은 이유로 별도 블록이라 그 선례를 따랐다. 잡는 metric은 EngravedScore 비교가 아니라 highlighter 자신의 `stats.visited`(§19.2가 이미 "만진 요소 수"라 이름 붙인 대리 지표) | MUTATIONS 배열에 억지로 끼워 넣고 `l2()`에 새 특수 케이스를 추가 — sync에는 EngravedScore가 없어 개념적으로 맞지 않고, l2.js에 layout과 무관한 코드를 심는 것도 어색하다 | §40.2 |
| G4-F1-2 | **`legacy-geometry.js`의 metric은 다섯 개로 제한한다**(`eg.clip.count`, `eg.overlap.head_head`, beam·tie·tuplet 구조 개수) — 일반 SVG geometry pass가 그래프 협조 없이 잴 수 있는 것만. A43의 "모든 범주"는 이 다섯 개를 뜻하는 것으로 읽었다(G04 §21.5 자체가 "옛 SVG에서 셀 수 있는 것"이라고 범위를 좁혀 적었음) | L1의 모든 `eg.*` metric을 legacy SVG에서 흉내 내기 — L1은 그래프와 ledger를 대조하는 것이라, 그래프를 읽지 않는 legacy 쪽에서는 애초에 잴 수 없는 것들이 대부분이다 | §40.3 |
| G4-F1-3 | **A43 판정 규칙: 결함 개수(clip, overlap)는 "G4 ≤ legacy", 구조 개수(beam, tie, tuplet)는 "G4 ≥ legacy".** 다만 tuplet 범주는 조사 뒤 이 규칙 자체가 이 범주에 안 맞는다고 §40.3에 기록했다(legacy가 그래프의 `show.number:'none'` 지시를 무시하는 것으로 보여, 개수가 많은 쪽이 오히려 결함일 수 있다) — 도구의 판정 규칙은 고치지 않고 이 사실을 있는 그대로 적었다("조용히 patch하지 않는다"는 브리프의 지시대로) | tuplet 범주를 도구에서 빼거나 판정 방향을 뒤집기 — 도구 자체를 이 세션이 임의로 재해석해 손대는 것이라, 발견을 그대로 보고하는 브리프의 원칙에 어긋난다 | §40.3 |
...
| G4-F2-7 | **인쇄 명령은 판각기가 그 Score를 그렸을 때만 보인다** (리뷰 R2): 앱의 `engraveView`가 판각기 자신의 paint 결과를 Score마다 WeakMap에 적고(`'drawn'` → true, `'legacy'` → false, fallback은 sticky, 기다림은 적지 않음), 값이 바뀌면 앱을 한 번 다시 그린다; `showPrintControls = S.wholeScore && engraveWanted(...) && engraveDrew(S.score)`. 그래도 인쇄가 실패하거나 파일을 못 불러오면 토스트 (`tx`, ko·ja·zh) | 원천을 다시 resolve해 미리 확인 (비싸고 판각기가 이미 아는 것을 되풀이), 버튼은 두고 실패만 알림 (누를 수 없는 명령을 보여 줌), `PPP.engraveStats.bySong`의 id로 판단 (fallback만 적고 "그렸다"는 모름; id가 같은 다른 Score 객체) | G04 §43.10.3; `print-check.js` 공유 seed 7곡, 부정 대조 4 FAILED |
| G4-F2-8 | **"Engraving…"을 `tx()`로** (리뷰 m1): 앱의 기다림 표시, `page.js`의 것(새 env `waitText`), ScoreView 자신의 VexFlow 기다림 표시는 한 줄의 `G4f-2: was … >>> … <<< G4f-2` 표시로 — A45 test가 표시에서 원래 글자를 되돌려 기존 hash 셋을 그대로 확인 | ScoreView의 표시는 두기 (flip 뒤 모든 사용자가 보는 영어), 클래스를 고치고 A45 hash를 새로 씀 (무엇이 바뀌었는지 말하지 못함) | G04 §43.10.1; `app.test.js` A45 |
| G4-F2-9 | **4× 재생의 §16.3 (50 ms 넘는 막힘 없음)은 inline 뒤에도 legacy처럼 지키지 못함을 기록한다** — 1×는 long task 0, 4×는 `<use>`보다 훨씬 낫고 대체로 legacy 이상이지만 0은 아니다 | 4× 수치를 부하 탓으로 빼기 | G04 §43.10.2 |
