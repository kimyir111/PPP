...
| G4-E7 | **§15.3의 60% 채움 세로 맞춤(페이지가 그만큼 차면 system 사이를 늘려 아래까지 맞춤)은 G4f로 미룬다.** 이 브리프의 명시적 Scope 목록에 없고(§15.5·§17.1만 열거), 지금은 항상 "위에 붙임" 쪽으로 동작한다 — 실제로 본 6곡 모두 첫 페이지가 92% 안팎까지 차 시각적으로 어색하지 않았다(§39.10) | 이번 세션에 구현 — 페이지마다 남는 여백을 언제 "60% 이상"으로 볼지, 어떻게 고르게 늘릴지 설계가 더 필요해 시간 안에 안전하게 끝내기 어려움 | §39.1, §39.11 |
